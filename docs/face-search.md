...
Face search is optional, gallery-scoped, and not an identity system. It is designed to help a visitor find **possible matches** in one gallery; it must never be described as recognition, identification, or a cross-gallery profile. HTTP paths use `/galleries`; `eventId` remains only an internal parameter and D1 relationship name.
...
Search is strictly per gallery. The Vectorize namespace contains the event ID and the Worker joins every candidate back to D1 with that same event ID, so a result from another gallery cannot be returned. For each direct match with a reliable capture timestamp, nearby search returns at most the four chronological photos immediately before and four immediately after it, and only inside a five-minute window. It does not use upload/creation time or a broad manually assigned moment label; a photo without a parsed capture timestamp produces no nearby context. After a successful search, the browser keeps sanitized event-photo references for direct matches and nearby moments in event-keyed `sessionStorage`; this restores the two result carousels after closing the viewer and powers the gallery's **All photos / Found for me** view for the current browser session. Stored references contain only photo ID, revision, thumbnail URL, capture time, and moment ID—not the selfie, crop, embedding, similarity score, or vector ID. The gallery keeps possible matches and nearby moments in labelled groups so temporal context is never presented as a facial match.
...
`wrangler.jsonc` binds the production `cadrora-face-index` and isolated `cadrora-preview-face-index`, both configured at 128 dimensions with cosine similarity. Each must have the `partition_id` string metadata index. The official `CADRORA_SEED_DEMO=true` flow uploads checksum-verified model objects, creates the fictional public **Retrouvez vos photos** gallery, detects every clear face in its generated photos, and upserts the resulting SFace embeddings.

The public find page offers downloadable portraits of Amelia and Daniel, two fictional generated guests. Demo seeding processes those portraits locally as release assertions and fails if either has no result above the production cosine threshold. The portrait embeddings are used only for that transient assertion and are never uploaded; only embeddings derived from the generated gallery photos enter Vectorize. Results remain **possible matches**, not identity claims. A real event needs owner-authorized photo indexing through the authenticated admin route before it can return possible matches.
