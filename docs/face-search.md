...
Search is strictly per gallery. The Vectorize namespace contains the event ID and the Worker joins every candidate back to D1 with that same event ID, so a result from another gallery cannot be returned. After a successful search, the browser keeps sanitized event-photo references for direct matches and nearby moments in event-keyed `sessionStorage`; this restores the two result carousels after closing the viewer and powers the gallery's **All photos / Found for me** view for the current browser session. Stored references contain only photo ID, revision, thumbnail URL, capture time, and moment ID—not the selfie, crop, embedding, similarity score, or vector ID. The gallery keeps possible matches and nearby moments in labelled groups so temporal context is never presented as a facial match.
