...
In protected galleries, a photo heart is a single D1-backed shared state. A same-origin, grant-checked Worker write changes only a published photo in that gallery. Public-gallery reads mask that state; public hearts stay in each visitor's browser storage and never call the write route. No visitor identity or reaction count is stored. A per-gallery owner switch closes private retouch selection while preserving the existing D1 queue.
