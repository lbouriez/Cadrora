...
| `VITE_GOOGLE_MAPS_EMBED_KEY` | Optional restricted public Google Maps Embed API key | No | Google Cloud Console → Maps Embed API → Credentials. Enable billing on that separate Google Cloud project, restrict the key to your site's HTTP referrers and the Maps Embed API, then add it only if you want the interactive contact map. Google currently lists Embed API usage as free, but the billing-account requirement remains. The iframe loads only after a visitor clicks to display it. |
