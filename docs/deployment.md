...
Use this path when `lbouriez/Cadrora` (or a fork you maintain) must remain the only source repository. Do **not** use the Deploy Button: Cloudflare's button creates a clone in a Git provider account. The labels below were cross-checked against Cloudflare's [Workers Builds](https://developers.cloudflare.com/workers/ci-cd/builds/), [Build configuration](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/), [R2](https://developers.cloudflare.com/r2/get-started/), and [Turnstile](https://developers.cloudflare.com/turnstile/get-started/widget-management/dashboard/) guides on 2026-09-25. Cloudflare may present a different setup form by account or entry point; the new-account Atelier Giulia UI and screenshots remain [unverified](technical/deployment-rehearsal-2026-09-25.md), so do not mistake this for a completed visual walkthrough.
...
2. **Enable R2 once.** Open **Storage & databases → R2 → Overview**. If R2 is not active, the account owner must review the [R2 subscription and checkout](https://developers.cloudflare.com/r2/get-started/) before continuing; included free usage does not mean the checkout step can be skipped. `npx wrangler r2 bucket list` may validly return an empty list; error `10042` means R2 is disabled.
...
5. **Create Turnstile for the real hostname.** Open **Turnstile → Add widget**; enter a **Widget name**, add `cadrora.com` under **Hostname management**, choose **Managed** under **Widget mode**, then select **Create**. Copy the public **sitekey** and private **secret key** separately; keep the latter in a password manager. Add `www.cadrora.com` only if it will be served. Localhost test keys are not production keys.
...
| `TURNSTILE_SECRET_KEY` | The private **secret key** for the production Cadrora widget | **Yes** on a setup form with **Encrypt**; otherwise create a build secret | **Turnstile → Cadrora Production**. Copy the secret key from the widget details; never photograph or document its value. |
| `VITE_TURNSTILE_SITE_KEY` | The matching public **sitekey** for that widget | **No**: normal build variable | **Turnstile → Cadrora Production**. This value is intentionally sent to the browser build. |
...
1. In the Cloudflare Dashboard account switcher, choose **AtelierGiulia**; open the `ateliergiulia.com` zone and confirm **Active**. Open **Storage & databases → R2 → Overview**. If Cloudflare asks to add an R2 subscription, review its checkout and possible future usage charges before accepting; this is a manual billing decision. Cloudflare's Custom Domain operation will later arrange DNS/TLS for the exact hostname; it does not transfer a domain from another account. Do not reuse Cadrora's D1, buckets, Vectorize index, Turnstile keys, admin credentials, or API token.
2. Open **Turnstile → Add widget**. Enter a descriptive **Widget name**, add `ateliergiulia.com` under **Hostname management**, choose **Managed** for **Widget mode**, and select **Create**. Copy its public **sitekey** and private **secret key** separately. For a user API token, open **My Profile → API Tokens → Create Token**, choose a custom token, scope it to the AtelierGiulia account and `ateliergiulia.com` zone, grant Zone Read, D1/R2/Vectorize write, Workers product write, and Workers Routes Write. Review the summary, select **Create Token**, and store the once-shown value in a password manager. The [current Cloudflare token guide](https://developers.cloudflare.com/fundamentals/api/get-started/create-token/) is authoritative if a permission label differs. Copy the account ID from the AtelierGiulia account overview. Never use a workstation's implicit Wrangler login here: it may select a different account.
3. Generate a **new**, stable admin password/verifier/pepper on a trusted workstation with `npm ci` and `npm run setup:admin-credentials`. Store the password in Giulia's password manager. Do not put the password in GitHub; only its `ADMIN_SECRET_HASH` and matching `AUTH_PEPPER` go to encrypted secrets. A new pepper on each release would invalidate login and protected-gallery verifiers.
4. In GitHub → repository **Settings → Environments**, open the already-created `atelier-giulia-production` environment (a fork must select **New environment → Configure environment**). Under **Environment secrets**, use **Add Secret** for `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, `ADMIN_SECRET_HASH`, `AUTH_PEPPER`, and `TURNSTILE_SECRET_KEY`. Under **Environment variables**, use **Add Variable** for the public `VITE_TURNSTILE_SITE_KEY`. Add an approval reviewer if desired. Under repository **Settings → Secrets and variables → Actions → Variables**, create `ATELIER_GIULIA_DEPLOY_ENABLED` with value `true` only after every item is ready. This gate must be a **repository** variable because the deploy job reads it before entering the environment. See [GitHub's environment-screen guide](https://docs.github.com/en/actions/how-tos/deploy/configure-and-manage-deployments/manage-environments). Do not paste secret values into workflow YAML, logs, issues, or screenshots.
