...
1. In Cloudflare, switch explicitly to the **AtelierGiulia** account and confirm `ateliergiulia.com` is an **active** zone. Enable R2 once if the dashboard requests it. Cloudflare's Custom Domain operation will attach the Worker and arrange DNS/TLS for the exact hostname; it does not transfer a domain from another account. Do not reuse Cadrora's D1, buckets, Vectorize index, Turnstile keys, admin credentials, or API token.
2. Create a Cloudflare Turnstile widget valid for `ateliergiulia.com`. Keep its public **site key** and private **secret key** distinct. Create a Cloudflare API token scoped to the AtelierGiulia account/zone with write access for D1, R2, Vectorize and the Workers product plus **Workers Routes Write** for the zone (needed for a Custom Domain), and zone read. Cloudflare's [current Workers permission guide](https://developers.cloudflare.com/workers/authorization/workers/) is authoritative if the role labels in the dashboard differ. Copy the account ID from that account's overview. Never use a workstation's implicit Wrangler login here: it may select a different Cloudflare account.
...
4. In GitHub → repository **Settings → Environments**, create `atelier-giulia-production`. Add an approval reviewer if desired. Add the environment secrets `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, `ADMIN_SECRET_HASH`, `AUTH_PEPPER`, and `TURNSTILE_SECRET_KEY`; add environment variable `VITE_TURNSTILE_SITE_KEY` (public key). GitHub's **repository** variable `ATELIER_GIULIA_DEPLOY_ENABLED` is the off/on gate: leave it unset while preparing. The deploy job reads this gate before it enters the environment; an environment-scoped gate will not work. Do not paste secret values into workflow YAML, logs, issues, or screenshots.
5. Push reviewed code to `main`; the action first runs checks and builds **both** profiles. Once the account, zone, widget, credentials and GitHub Environment are ready, set repository variable `ATELIER_GIULIA_DEPLOY_ENABLED=true`, then run **Actions → Atelier Giulia production → Run workflow** on `main`. The deploy job provisions/reuses `cadrora-atelier-giulia` Worker/D1, two private R2 buckets and a Vectorize index, applies migrations and the guarded brand seed, uploads verified models, sets Worker secrets, binds `ateliergiulia.com`, then checks three HTTPS routes. Re-running the same workflow reuses those resources.
