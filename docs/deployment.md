...
4. **Create the admin credential.** Run `npm ci` then `npm run setup:admin-credentials` on a trusted workstation. Store `ADMIN_PASSWORD` from the ignored artifact in a password manager. Cloudflare gets only `ADMIN_SECRET_HASH`—never the password or the artifact.
