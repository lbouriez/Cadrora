...
`npm run deploy` builds the production target, runs `wrangler d1 migrations apply DB --remote`, and invokes Wrangler with a temporary three-secret file. Release-script Wrangler processes set `CI=true` after the explicit `--confirm` fence so a Workers Build cannot stall on a D1 terminal prompt. The manual release commands require an explicit target and `--confirm`; neither the local setup script nor diagnostic mode calls Cloudflare. Do not treat a command exit alone as evidence that custom hostnames, secrets, model objects, or Cron delivery work.
