...
The official Cadrora showcase also exposes a clearly labelled **demo** sign-in when the deployment explicitly sets `CADRORA_SEED_DEMO=true`; the default is off. Its published credentials create a separate one-hour, stateless `read-only` session; they are intentionally not a secret and are not an operator credential. The Worker—not the disabled UI—allows only session, event-list, publication-readiness, and usage reads. Every mutation and every unrecognized admin read returns `DEMO_READ_ONLY` before D1 or R2 is touched. The reserved `demo-private` gallery password has a similarly narrow showcase-only bypass; all real protected events use HMAC verification with the secret pepper. Do not reuse demonstration credentials or enable the showcase gate for an operator deployment.
