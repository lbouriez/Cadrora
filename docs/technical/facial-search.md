...
The result experience remains event-scoped after the API call. The client stores sanitized direct-match and, when the gallery owner enables it, nearby-moment photo references for that event in `sessionStorage`, so the visitor can return from the viewer to the same find results and switch the gallery between all photos and **Found for me** during the current browser session. Each reference is limited to photo ID, revision, thumbnail URL, capture time, and moment ID; the visitor image, crop, embedding, score, and vector ID are never stored. Related-photo requests are issued only when `nearbySearchEnabled` is true, automatically for every newly returned direct match, and deduplicated across moments. D1 selects at most four immediately preceding and four immediately following same-gallery photos whose parsed capture instants are no farther than five minutes from the match. `moment_id`, upload time, and creation time do not expand that window; a missing capture instant returns no related photos. Disabling face search also disables nearby search. The gallery renders nearby moments in their own labelled group because temporal proximity is not a facial match.
...
The find route dynamically imports `onnxruntime-web`. WASM is the default; WebGPU is opt-in and uses the package's JSPI entry only when both WebGPU and WebAssembly JSPI are available, otherwise it falls back to WASM. This keeps both emitted runtime binaries below Cloudflare's 25 MiB per-static-asset deployment limit; do not switch the WebGPU path back to the larger Asyncify entry without changing the hosting design. Vite resolves and emits the ONNX package WASM assets. Model responses are downloaded once, checked for size and SHA-256, then stored in a versioned browser Cache API cache before session creation. SFace input uses a least-squares affine transform across all five YuNet landmarks, then raw `0–255` RGB NCHW pixels matching OpenCV `blobFromImage(..., scale=1, swapRB=true)`. The output alone is L2-normalized for cosine search.

With `CADRORA_SEED_DEMO=true`, `scripts/demo/faceVectors.mjs` runs the same pinned YuNet/SFace pair against the tracked fictional gallery media during deployment. It writes ignored NDJSON and SQL artifacts, verifies that Amelia, Daniel, and Nora's test portraits each exceed the production match threshold, requires Nora's portrait to match one of her two gallery photos, rejects matches against the five Malik photos for all three portraits, uploads only gallery-derived embeddings to Vectorize, and replaces the reserved demo event's D1 face references. A photo with no clear YuNet candidate is reported and skipped; any test portrait without a match fails the seed. The generated vertical Amelia and Nora photos are indexed alongside the landscape photos to exercise mixed-aspect gallery and viewer paths without treating nearby images as face matches.
