...
The result experience remains event-scoped after the API call. The client stores sanitized direct-match and nearby-moment photo references for that event in `sessionStorage`, so the visitor can return from the full-screen viewer to the same find results and switch the gallery between all photos and **Found for me** during the current browser session. Each reference is limited to photo ID, revision, thumbnail URL, capture time, and moment ID; the visitor image, crop, embedding, score, and vector ID are never stored. Related-photo requests are issued automatically for every newly returned direct match and deduplicated across moments. The gallery renders nearby moments in their own labelled group because temporal proximity is not a facial match.
