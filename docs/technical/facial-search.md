...
With `CADRORA_SEED_DEMO=true`, `scripts/demo/faceVectors.mjs` runs the same pinned YuNet/SFace pair against the tracked fictional gallery media during deployment. It writes ignored NDJSON and SQL artifacts, verifies that the Amelia and Daniel test portraits each exceed the production match threshold, rejects matches against the seven Malik/Nora negative-control photos, uploads only gallery-derived embeddings to Vectorize, and replaces the reserved demo event's D1 face references. A photo with no clear YuNet candidate is reported and skipped; either portrait having no match fails the seed. The generated vertical Amelia and Nora portraits are indexed alongside the landscape photos to exercise mixed-aspect gallery and viewer paths without treating nearby images as face matches.
