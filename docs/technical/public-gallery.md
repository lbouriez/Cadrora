...
- The gallery renders lazy responsive image sources and provides Escape, arrow-key, button, and horizontal-swipe viewer navigation. Shared SVG icon controls remain geometrically centered and hover states must not replace positional transforms. It links downloads only when the event permits them and a download variant exists.
- `showPhotoMetadata` is an event-level owner setting that defaults off. When enabled, the viewer information control reveals only filename, event-local capture time, and dimensions already returned by the authorized photo API.
- A successful face search stores only up to the default 2,000-photo event cap as matched photo IDs in event-keyed `sessionStorage`. The gallery exposes an **All photos / Found for me** control, automatically loads paginated results needed by that filter, and preserves the filter while the viewer is open. It stores no selfie, crop, embedding, score, or nearby-moment result.
