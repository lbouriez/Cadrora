...
- Visitor download selection is gallery-local and works on loaded photos, including the filtered "Found for me" view. One photo uses the authorized media attachment directly. For multiple photos, `downloadPhotos.ts` streams into a unique child directory through `showDirectoryPicker` when supported, or lazily constructs a bounded ZIP on the visitor device. Do not set the picker `startIn` to Downloads: Chromium may reject that special folder itself as sensitive. The UI explains how to select an ordinary subfolder and surfaces picker rejection instead of silently swallowing its `AbortError`; ZIP remains the fallback. The ZIP accepts at most 100 photos/250 MB, keeps the selection on failure, and offers the finished Blob through a user-clicked link. Every media URL is constrained to the same-origin, exact gallery/photo/revision `download` route; the Worker rechecks authorization for each request. Keep the folder picker call inside the original click activation.
...
- The shared `.public-main :where(a:not(.button))` link styling must never reset one side of a border. Gallery back links supply their own complete themed border; a later `border-bottom: 0` declaration silently removed only their bottom edge.
