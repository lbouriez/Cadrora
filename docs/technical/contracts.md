...
[`ADR-012`](../decisions/ADR-012-gallery-directory-and-progressive-photos.md) adds owner-controlled directory inclusion, default on. The public listing includes only published, online galleries with `showOnGalleryPage=true`, ordered across public and protected cards by creation time descending. The setting is not access control; direct links retain their normal authorization. The gallery header is compact and text-only. Its photo API remains cursor-paginated while the client requests the next page near the viewport and progressively upgrades lazy tile images from the smallest prepared variant to a responsive larger one. A manual load-more button remains available.
