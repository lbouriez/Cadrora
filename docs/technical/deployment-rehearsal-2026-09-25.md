...
- **Storage & databases → R2 Object Storage** still opens **Get started with R2**. The subscription screen showed $0 due immediately/monthly but an existing payment card, automatic renewal, and usage-based charges above the allowance. The owner must click **Add R2 subscription to my account** personally after reviewing those terms. No R2 subscription has been confirmed.
...
- `npm run check`, `npm run test`, and `npm run build` pass with the account/zone preflight tests. `node scripts/instances/deploy.mjs --help` prints the expected syntax. A real Wrangler provision/deploy cannot be exercised until the AtelierGiulia token and R2 subscription are available.
...
1. The owner completes the R2 subscription on **Storage & databases → R2 Object Storage → Get started with R2 → Add R2 subscription to my account**. Reopen R2 and confirm the bucket overview appears.
2. Finish the pending **Manage account → Account API tokens → Create Token** flow. The account-owned token review screen was reached with `ateliergiulia.com` **Zone Read** and **Workers Routes Write**, plus account-scoped **D1 Write**, **Vectorize Write**, **Workers R2 Storage Write**, and **Workers Admin**. Do not click **Create token** without the owner's action-time confirmation. Transfer the once-shown value only to GitHub Environment secret `CLOUDFLARE_API_TOKEN` and a password manager.
3. Enable the **repository** gate and run the Action only after R2 and the token are ready. Verify preflight occurs **before** any Wrangler creation, then check terminal GitHub/Cloudflare status and exact commit.
4. Open all public routes and `/admin/login`; check admin authentication, D1 site settings, R2 media privacy, model route, gallery flow, and mobile rendering. Confirm DNS, certificate, HTTP→HTTPS redirect, then enable HSTS conservatively and verify its response header.
5. **Separately** rehearse the README's Workers Builds import path in a clean account or disposable isolated project if the claim “anyone can follow this tutorial” is to be validated end to end. Do not mark that path tested merely because the Atelier GitHub Action succeeded.
