...
The Worker rejects import declarations over `MAX_PHOTOS_PER_EVENT` and media uploads over `MAX_STORAGE_BYTES`. It fails closed if either binding is invalid. A stopped upload leaves an idempotent partial object at its derived key and an unfinished journal chunk; it is safe to resume. Cancelling stops browser work and fences every photo declared by that import; periodic maintenance removes its provider objects.
