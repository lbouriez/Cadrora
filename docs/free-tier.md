...
- **R2 Standard:** 2 MB *total across all image variants per photo*, not 2 MB per object or an original file, plus 0.1 GB for private models/overhead. Storage is `photos × 0.002 + 0.1` GB-month. Billable storage is approximated as `ceil(max(0, GB-month − 10)) × $0.015`; Cloudflare rounds R2 usage to whole billing units. The 10 GB-month allowance applies to the whole account, not each bucket. Real image sizes can be much larger.
