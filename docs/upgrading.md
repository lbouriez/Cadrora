...
Migrations `009_gallery_deletion.sql`, `010_site_languages.sql`, and `011_owner_quotas.sql` added deletion fencing/jobs, the enabled-language list, and nullable owner self-limits. Deploy them before code that reads `events.deleting_at`, `site_settings.enabled_languages`, or the retained storage/face limit columns; the checked-in release script does this automatically. Existing sites start with both French and English enabled and use the deployment storage/face ceilings until an owner saves lower limits. [ADR-017](decisions/ADR-017-remove-gallery-count-limit.md) removes the gallery-count setting and enforcement. Deploy `021_remove_gallery_limit.sql` only after the updated Worker is live; it drops the former D1 gallery-limit column.
