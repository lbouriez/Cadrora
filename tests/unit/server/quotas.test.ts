...
describe('owner quota guardrails', () => {
  ...
  it('reports D1-backed usage for galleries, media variants, and indexed faces', async () => {
    const prepare = vi.fn((query: string) => ({
      first: vi.fn().mockResolvedValue({
        value: query.includes('FROM events') ? 3 : query.includes('photo_variants') ? 750000 : 24,
      }),
    }));
