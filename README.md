...
5. The Contact page shows the configured details and service area without Google. To enable an embedded Google map, optionally supply the public build variable `VITE_GOOGLE_MAPS_EMBED_KEY` with a **Google Maps Embed API** key restricted to this website's HTTP referrers and to that API. Google requires a Cloud project with billing enabled even though the Embed API currently lists no usage charge; this is a separate, optional operator choice. The map iframe loads only after a visitor selects **Display Google Maps**. Without a key, the page offers an external Google Maps link. The radius is shown as an approximate travel distance, not a precise map boundary. See [public website configuration](docs/technical/public-website.md).
