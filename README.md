...
Cadrora is an open-source, self-hosted photographer website and event photo gallery for one photographer or family. Its static React surface presents the photographer and services, publishes direct contact details, and hosts public or protected event galleries. A Cloudflare Worker backed by D1 and private R2 buckets handles the private application flows. Facial search is optional and strictly event-scoped: it is enabled for the official fictitious showcase only, and a visitor's selfie stays in their browser. Successful possible-match IDs can be reviewed through a temporary **All photos / Found for me** gallery filter; no selfie or embedding is stored with that filter.
