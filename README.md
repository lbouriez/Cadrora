...
9. Select **Deploy**, wait for the production build to succeed, then attach the custom domain. Verify `/`, `/services`, `/galleries`, `/contact`, `/privacy`, `/api/v1/site`, and `/admin/login` on the Workers hostname before repeating the checks on the domain. The legacy browser route `/events` redirects to `/galleries`; internal `/api/v1/events/*` routes remain stable for compatibility.
